  ...
  try {
    ...
    if (cart.products.length > 0) {
      ...
      for (let Product of cart.products) {
        const productInfor = await product
          .findOne({
            _id: Product.productId,
          })
          .select(
            "thumbnail title slugTitle price discountPercentage quantity"
          );
        productInfor.newPrice = (
          (1 - productInfor.discountPercentage / 100) *
          productInfor.price
        ).toFixed(0);
        Product.totalPrice =
          parseInt(productInfor.newPrice) * parseInt(Product.quantity);
...
module.exports.addPost = async (req, res) => {
  try {
    ...
    const productCurrent = cart.products.find(
